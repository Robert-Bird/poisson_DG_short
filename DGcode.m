...
% plotting the data 
function plot_u(etpl,ed,coord,u,dx)
    point_data = zeros(size(etpl,1)*size(etpl,2),1);
    coord_data = zeros(size(etpl,1)*size(etpl,2),2);
    c=0;
    for el = 1:size(etpl,1)
        for loc_node = 1:4
            c=c+1;
            node = etpl(el,loc_node);
            dof = ed(el,loc_node);
            coord_data(c,:) = coord(node,:);
            point_data(c)   = u(dof);
        end
    end
    plotsurf_vectordata(coord_data(:,1),coord_data(:,2),point_data,dx)
end

function plotsurf_vectordata(x,y,z,dx)
